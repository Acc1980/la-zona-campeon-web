  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        {/* Stats strip */}
        <div className="absolute bottom-0 left-0 right-0 bg-dark-900/80 backdrop-blur-sm border-t border-dark-700">
          <div className="section-container py-5">
            <div className="grid grid-cols-3 gap-4 max-w-lg">
              {[
                { num: "10+", label: "Capítulos por manual" },
                { num: "5+", label: "Deportes cubiertos" },
                { num: "100%", label: "En Español" },
              ].map((stat) => (
                <div key={stat.label} className="text-center sm:text-left">
                  <div className="font-display font-black text-2xl md:text-3xl text-gold-500">{stat.num}</div>
                  <div className="text-dark-300 text-xs uppercase tracking-wider mt-0.5">{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
